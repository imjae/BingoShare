...
type PackedBoard = [version: 1, title: string, size: BoardSize, shuffle: 0 | 1, freeCenter: 0 | 1, cells: string[]]

function pack(board: Board): PackedBoard {
  return [1, board.title, board.size, board.shuffle ? 1 : 0, board.freeCenter ? 1 : 0, board.cells]
}
...
function unpack(packed: unknown): Board {
  ...
  const [version, title, size, shuffle, freeCenter, cells] = packed as PackedBoard
  if (version !== 1) {
